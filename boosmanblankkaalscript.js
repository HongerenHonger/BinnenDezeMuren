...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId10 = getQueryParam("id");
  ...
  if (!ervaringId10) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId10);
    bindStapB(ervaringId10);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput10").value.trim();
    const ervaring = document.getElementById("ervaringinput10").value.trim();
    const htmlbestand = "boosmanblankkaal";
...
async function laadEnToonErvaringEnReacties(ervaringId10) {
  ...
  document.getElementById("toonEmotie10").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring10").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId10)
  ...
  const lijstContainer = document.getElementById("reactieLijst10");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst10");
...
function bindStapB(ervaringId10) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam10 = document.getElementById("naaminput10").value.trim();
    const reactie10 = document.getElementById("reactieinput10").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam10;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie10;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput10").value = "";
    document.getElementById("reactieinput10").value = "";
  });
}
