...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId25 = getQueryParam("id");
  ...
  if (!ervaringId25) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId25);
    bindStapB(ervaringId25);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput25").value.trim();
    const ervaring = document.getElementById("ervaringinput25").value.trim();
    const htmlbestand = "stressmanblankbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId25) {
  ...
  document.getElementById("toonEmotie25").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring25").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId25)
  ...
  const lijstContainer = document.getElementById("reactieLijst25");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst25");
...
function bindStapB(ervaringId25) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam25 = document.getElementById("naaminput25").value.trim();
    const reactie25 = document.getElementById("reactieinput25").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam25;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie25;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput25").value = "";
    document.getElementById("reactieinput25").value = "";
  });
}
