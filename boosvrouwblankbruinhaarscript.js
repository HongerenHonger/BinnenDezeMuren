...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId14 = getQueryParam("id");
  ...
  if (!ervaringId14) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId14);
    bindStapB(ervaringId14);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput14").value.trim();
    const ervaring = document.getElementById("ervaringinput14").value.trim();
    const htmlbestand = "boosvrouwblankbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId14) {
  ...
  document.getElementById("toonEmotie14").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring14").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId14)
  ...
  const lijstContainer = document.getElementById("reactieLijst14");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst14");
...
function bindStapB(ervaringId14) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam14 = document.getElementById("naaminput14").value.trim();
    const reactie14 = document.getElementById("reactieinput14").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam14;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie14;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput14").value = "";
    document.getElementById("reactieinput14").value = "";
  });
}
