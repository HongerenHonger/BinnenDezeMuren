...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId12 = getQueryParam("id");
  ...
  if (!ervaringId12) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId12);
    bindStapB(ervaringId12);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput12").value.trim();
    const ervaring = document.getElementById("ervaringinput12").value.trim();
    const htmlbestand = "boosvrouwaziaatbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId12) {
  ...
  document.getElementById("toonEmotie12").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring12").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId12)
  ...
  const lijstContainer = document.getElementById("reactieLijst12");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst12");
...
function bindStapB(ervaringId12) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam12 = document.getElementById("naaminput12").value.trim();
    const reactie12 = document.getElementById("reactieinput12").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam12;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie12;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput12").value = "";
    document.getElementById("reactieinput12").value = "";
  });
}
