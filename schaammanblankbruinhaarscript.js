...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId17 = getQueryParam("id");
  ...
  if (!ervaringId17) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId17);
    bindStapB(ervaringId17);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput17").value.trim();
    const ervaring = document.getElementById("ervaringinput17").value.trim();
    const htmlbestand = "schaammanblankbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId17) {
  ...
  document.getElementById("toonEmotie17").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring17").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId17)
  ...
  const lijstContainer = document.getElementById("reactieLijst17");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst17");
...
function bindStapB(ervaringId17) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam17 = document.getElementById("naaminput17").value.trim();
    const reactie17 = document.getElementById("reactieinput17").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam17;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie17;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput17").value = "";
    document.getElementById("reactieinput17").value = "";
  });
}
