...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId7 = getQueryParam("id");
  ...
  if (!ervaringId7) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId7);
    bindStapB(ervaringId7);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput7").value.trim();
    const ervaring = document.getElementById("ervaringinput7").value.trim();
    const htmlbestand = "blijvrouwzwartbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId7) {
  ...
  document.getElementById("toonEmotie7").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring7").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId7)
...
function bindStapB(ervaringId7) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam7 = document.getElementById("naaminput7").value.trim();
    const reactie7 = document.getElementById("reactieinput7").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam7;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie7;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput7").value = "";
    document.getElementById("reactieinput7").value = "";
  });
}
