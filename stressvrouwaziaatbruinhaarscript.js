...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId28 = getQueryParam("id");
  ...
  if (!ervaringId28) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId28);
    bindStapB(ervaringId28);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput28").value.trim();
    const ervaring = document.getElementById("ervaringinput28").value.trim();
    const htmlbestand = "stressvrouwaziaatbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId28) {
  ...
  document.getElementById("toonEmotie28").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring28").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId28)
  ...
  const lijstContainer = document.getElementById("reactieLijst28");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst28");
...
function bindStapB(ervaringId28) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam28 = document.getElementById("naaminput28").value.trim();
    const reactie28 = document.getElementById("reactieinput28").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam28;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie28;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput28").value = "";
    document.getElementById("reactieinput28").value = "";
  });
}
