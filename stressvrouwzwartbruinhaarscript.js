...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId31 = getQueryParam("id");
  ...
  if (!ervaringId31) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId31);
    bindStapB(ervaringId31);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput31").value.trim();
    const ervaring = document.getElementById("ervaringinput31").value.trim();
    const htmlbestand = "stressvrouwzwartbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId31) {
  ...
  document.getElementById("toonEmotie31").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring31").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId31)
  ...
  const lijstContainer = document.getElementById("reactieLijst31");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst31");
...
function bindStapB(ervaringId31) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam31 = document.getElementById("naaminput31").value.trim();
    const reactie31 = document.getElementById("reactieinput31").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam31;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie31;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput31").value = "";
    document.getElementById("reactieinput31").value = "";
  });
}
