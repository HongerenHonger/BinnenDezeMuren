...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId29 = getQueryParam("id");
  ...
  if (!ervaringId29) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId29);
    bindStapB(ervaringId29);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput29").value.trim();
    const ervaring = document.getElementById("ervaringinput29").value.trim();
    const htmlbestand = "stressvrouwblankblondhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId29) {
  ...
  document.getElementById("toonEmotie29").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring29").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId29)
  ...
  const lijstContainer = document.getElementById("reactieLijst29");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst29");
...
function bindStapB(ervaringId29) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam29 = document.getElementById("naaminput29").value.trim();
    const reactie29 = document.getElementById("reactieinput29").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam29;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie29;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput29").value = "";
    document.getElementById("reactieinput29").value = "";
  });
}
