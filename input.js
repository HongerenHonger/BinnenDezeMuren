...
document.addEventListener("DOMContentLoaded", async () => {
  let ervaringId = getQueryParam("id");
  htmlbestand = getQueryParam("htmlbestand");
  const sectieA = document.getElementById("stapA");
  const sectieB = document.getElementById("stapB");


  document.getElementById("stapA-image").src = `Images/${htmlbestand}.png`;
  document.getElementById("stapB-image").src = `Images/${htmlbestand}.png`;
  ...
  if (!htmlbestand) {
    ...
    const { data: ervaringRecord, error: errorFetch } = await supabase
      .from("ervaringen")
      .select("id, emotie, ervaring, htmlbestand")
      .eq("htmlbestand", htmlbestand)
      .single();

    if (ervaringRecord) {
      window.history.replaceState({}, '', `input.html?id=${ervaringRecord.id}&htmlbestand=${ervaringRecord.htmlbestand}`);
      ervaringId = ervaringRecord.id;
    }else {
