...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId15 = getQueryParam("id");
  ...
  if (!ervaringId15) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId15);
    bindStapB(ervaringId15);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput15").value.trim();
    const ervaring = document.getElementById("ervaringinput15").value.trim();
    const htmlbestand = "boosvrouwzwartbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId15) {
  ...
  document.getElementById("toonEmotie15").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring15").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId15)
  ...
  const lijstContainer = document.getElementById("reactieLijst15");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst15");
...
function bindStapB(ervaringId15) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam15 = document.getElementById("naaminput15").value.trim();
    const reactie15 = document.getElementById("reactieinput15").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam15;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie15;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput15").value = "";
    document.getElementById("reactieinput15").value = "";
  });
}
