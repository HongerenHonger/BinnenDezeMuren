...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId20 = getQueryParam("id");
  ...
  if (!ervaringId20) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId20);
    bindStapB(ervaringId20);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput20").value.trim();
    const ervaring = document.getElementById("ervaringinput20").value.trim();
    const htmlbestand = "schaamvrouwaziaatbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId20) {
  ...
  document.getElementById("toonEmotie20").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring20").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId20)
  ...
  const lijstContainer = document.getElementById("reactieLijst20");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst20");
...
function bindStapB(ervaringId20) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam20 = document.getElementById("naaminput20").value.trim();
    const reactie20 = document.getElementById("reactieinput20").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam20;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie20;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput20").value = "";
    document.getElementById("reactieinput20").value = "";
  });
}
