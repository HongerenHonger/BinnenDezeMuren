...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId23 = getQueryParam("id");
  ...
  if (!ervaringId23) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId23);
    bindStapB(ervaringId23);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput23").value.trim();
    const ervaring = document.getElementById("ervaringinput23").value.trim();
    const htmlbestand = "schaamvrouwzwartbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId23) {
  ...
  document.getElementById("toonEmotie23").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring23").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId23)
  ...
  const lijstContainer = document.getElementById("reactieLijst23");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst23");
...
function bindStapB(ervaringId23) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam23 = document.getElementById("naaminput23").value.trim();
    const reactie23 = document.getElementById("reactieinput23").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam23;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie23;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput23").value = "";
    document.getElementById("reactieinput23").value = "";
  });
}
