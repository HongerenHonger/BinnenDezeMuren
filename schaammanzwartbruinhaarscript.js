...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId19 = getQueryParam("id");
  ...
  if (!ervaringId19) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId19);
    bindStapB(ervaringId19);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput19").value.trim();
    const ervaring = document.getElementById("ervaringinput19").value.trim();
    const htmlbestand = "schaammanzwartbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId19) {
  ...
  document.getElementById("toonEmotie19").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring19").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId19)
  ...
  const lijstContainer = document.getElementById("reactieLijst19");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst19");
...
function bindStapB(ervaringId19) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam19 = document.getElementById("naaminput19").value.trim();
    const reactie19 = document.getElementById("reactieinput19").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam19;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie19;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput19").value = "";
    document.getElementById("reactieinput19").value = "";
  });
}
