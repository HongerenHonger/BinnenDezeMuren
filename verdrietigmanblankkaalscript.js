...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId34 = getQueryParam("id");
  ...
  if (!ervaringId34) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId34);
    bindStapB(ervaringId34);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput34").value.trim();
    const ervaring = document.getElementById("ervaringinput34").value.trim();
    const htmlbestand = "verdrietigmanblankkaal";
...
async function laadEnToonErvaringEnReacties(ervaringId34) {
  ...
  document.getElementById("toonEmotie34").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring34").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId34)
  ...
  const lijstContainer = document.getElementById("reactieLijst34");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst34");
...
function bindStapB(ervaringId34) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam34 = document.getElementById("naaminput34").value.trim();
    const reactie34 = document.getElementById("reactieinput34").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam34;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie34;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput34").value = "";
    document.getElementById("reactieinput34").value = "";
  });
}
