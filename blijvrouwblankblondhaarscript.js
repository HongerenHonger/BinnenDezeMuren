...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId5 = getQueryParam("id");
  ...
  if (!ervaringId5) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId5);
    bindStapB(ervaringId5);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput5").value.trim();
    const ervaring = document.getElementById("ervaringinput5").value.trim();
    const htmlbestand = "blijvrouwblankblondhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId5) {
  ...
  document.getElementById("toonEmotie5").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring5").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId5)
...
function bindStapB(ervaringId5) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam5 = document.getElementById("naaminput5").value.trim();
    const reactie5 = document.getElementById("reactieinput5").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam5;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie5;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput5").value = "";
    document.getElementById("reactieinput5").value = "";
  });
}
