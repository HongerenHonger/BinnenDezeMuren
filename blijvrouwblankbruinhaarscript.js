...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId6 = getQueryParam("id");
  ...
  if (!ervaringId6) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId6);
    bindStapB(ervaringId6);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput6").value.trim();
    const ervaring = document.getElementById("ervaringinput6").value.trim();
    const htmlbestand = "blijvrouwblankbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId6) {
  ...
  document.getElementById("toonEmotie6").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring6").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId6)
...
function bindStapB(ervaringId6) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam6 = document.getElementById("naaminput6").value.trim();
    const reactie6 = document.getElementById("reactieinput6").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam6;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie6;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput6").value = "";
    document.getElementById("reactieinput6").value = "";
  });
}
