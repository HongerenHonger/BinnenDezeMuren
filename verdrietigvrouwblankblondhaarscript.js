...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId37 = getQueryParam("id");
  ...
  if (!ervaringId37) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId37);
    bindStapB(ervaringId37);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput37").value.trim();
    const ervaring = document.getElementById("ervaringinput37").value.trim();
    const htmlbestand = "verdrietigvrouwblankblondhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId37) {
  ...
  document.getElementById("toonEmotie37").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring37").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId37)
  ...
  const lijstContainer = document.getElementById("reactieLijst37");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst37");
...
function bindStapB(ervaringId37) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam37 = document.getElementById("naaminput37").value.trim();
    const reactie37 = document.getElementById("reactieinput37").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam37;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie37;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput37").value = "";
    document.getElementById("reactieinput37").value = "";
  });
}
