...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId27 = getQueryParam("id");
  ...
  if (!ervaringId27) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId27);
    bindStapB(ervaringId27);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput27").value.trim();
    const ervaring = document.getElementById("ervaringinput27").value.trim();
    const htmlbestand = "stressmanzwartbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId27) {
  ...
  document.getElementById("toonEmotie27").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring27").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId27)
  ...
  const lijstContainer = document.getElementById("reactieLijst27");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst27");
...
function bindStapB(ervaringId27) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam27 = document.getElementById("naaminput27").value.trim();
    const reactie27 = document.getElementById("reactieinput27").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam27;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie27;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput27").value = "";
    document.getElementById("reactieinput27").value = "";
  });
}
