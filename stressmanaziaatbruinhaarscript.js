...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId24 = getQueryParam("id");
  ...
  if (!ervaringId24) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId24);
    bindStapB(ervaringId24);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput24").value.trim();
    const ervaring = document.getElementById("ervaringinput24").value.trim();
    const htmlbestand = "stressmanaziaatbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId24) {
  ...
  document.getElementById("toonEmotie24").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring24").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId24)
  ...
  const lijstContainer = document.getElementById("reactieLijst24");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst24");
...
function bindStapB(ervaringId24) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam24 = document.getElementById("naaminput24").value.trim();
    const reactie24 = document.getElementById("reactieinput24").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam24;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie24;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput24").value = "";
    document.getElementById("reactieinput24").value = "";
  });
}
