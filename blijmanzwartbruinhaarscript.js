...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId3 = getQueryParam("id");
  ...
  if (!ervaringId3) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId3);
    bindStapB(ervaringId3);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput3").value.trim();
    const ervaring = document.getElementById("ervaringinput3").value.trim();
    const htmlbestand = "blijmanzwartbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId3) {
  ...
  document.getElementById("toonEmotie3").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring3").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId3)
...
function bindStapB(ervaringId3) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam3 = document.getElementById("naaminput3").value.trim();
    const reactie3 = document.getElementById("reactieinput3").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam3;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie3;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput3").value = "";
    document.getElementById("reactieinput3").value = "";
  });
}
