...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId33 = getQueryParam("id");
  ...
  if (!ervaringId33) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId33);
    bindStapB(ervaringId33);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput33").value.trim();
    const ervaring = document.getElementById("ervaringinput33").value.trim();
    const htmlbestand = "verdrietigmanblankbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId33) {
  ...
  document.getElementById("toonEmotie33").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring33").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId33)
  ...
  const lijstContainer = document.getElementById("reactieLijst33");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst33");
...
function bindStapB(ervaringId33) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam33 = document.getElementById("naaminput33").value.trim();
    const reactie33 = document.getElementById("reactieinput33").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam33;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie33;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput33").value = "";
    document.getElementById("reactieinput33").value = "";
  });
}
