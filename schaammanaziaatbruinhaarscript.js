...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId16 = getQueryParam("id");
  ...
  if (!ervaringId16) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId16);
    bindStapB(ervaringId16);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput16").value.trim();
    const ervaring = document.getElementById("ervaringinput16").value.trim();
    const htmlbestand = "schaammanaziaatbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId16) {
  ...
  document.getElementById("toonEmotie16").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring16").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId16)
  ...
  const lijstContainer = document.getElementById("reactieLijst16");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst16");
...
function bindStapB(ervaringId16) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam16 = document.getElementById("naaminput16").value.trim();
    const reactie16 = document.getElementById("reactieinput16").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam16;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie16;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput16").value = "";
    document.getElementById("reactieinput16").value = "";
  });
}
