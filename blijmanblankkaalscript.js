...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId2 = getQueryParam("id");
  ...
  if (!ervaringId2) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId2);
    bindStapB(ervaringId2);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput2").value.trim();
    const ervaring = document.getElementById("ervaringinput2").value.trim();
    const htmlbestand = "blijmanblankkaal"; // Updated for this file
...
async function laadEnToonErvaringEnReacties(ervaringId2) {
  ...
  document.getElementById("toonEmotie2").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring2").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId2)
...
function bindStapB(ervaringId2) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam2 = document.getElementById("naaminput2").value.trim();
    const reactie2 = document.getElementById("reactieinput2").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam2;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie2;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput2").value = "";
    document.getElementById("reactieinput2").value = "";
  });
}
