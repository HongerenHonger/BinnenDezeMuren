...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId36 = getQueryParam("id");
  ...
  if (!ervaringId36) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId36);
    bindStapB(ervaringId36);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput36").value.trim();
    const ervaring = document.getElementById("ervaringinput36").value.trim();
    const htmlbestand = "verdrietigvrouwaziaatbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId36) {
  ...
  document.getElementById("toonEmotie36").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring36").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId36)
  ...
  const lijstContainer = document.getElementById("reactieLijst36");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst36");
...
function bindStapB(ervaringId36) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam36 = document.getElementById("naaminput36").value.trim();
    const reactie36 = document.getElementById("reactieinput36").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam36;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie36;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput36").value = "";
    document.getElementById("reactieinput36").value = "";
  });
}
