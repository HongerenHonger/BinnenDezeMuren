...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId13 = getQueryParam("id");
  ...
  if (!ervaringId13) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId13);
    bindStapB(ervaringId13);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput13").value.trim();
    const ervaring = document.getElementById("ervaringinput13").value.trim();
    const htmlbestand = "boosvrouwblankblondhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId13) {
  ...
  document.getElementById("toonEmotie13").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring13").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId13)
  ...
  const lijstContainer = document.getElementById("reactieLijst13");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst13");
...
function bindStapB(ervaringId13) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam13 = document.getElementById("naaminput13").value.trim();
    const reactie13 = document.getElementById("reactieinput13").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam13;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie13;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput13").value = "";
    document.getElementById("reactieinput13").value = "";
  });
}
