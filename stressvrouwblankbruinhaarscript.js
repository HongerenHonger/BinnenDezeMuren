...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId30 = getQueryParam("id");
  ...
  if (!ervaringId30) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId30);
    bindStapB(ervaringId30);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput30").value.trim();
    const ervaring = document.getElementById("ervaringinput30").value.trim();
    const htmlbestand = "stressvrouwblankbruinhaar";
...
async function laadEnToonErvaringEnReacties(ervaringId30) {
  ...
  document.getElementById("toonEmotie30").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring30").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId30)
  ...
  const lijstContainer = document.getElementById("reactieLijst30");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst30");
...
function bindStapB(ervaringId30) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam30 = document.getElementById("naaminput30").value.trim();
    const reactie30 = document.getElementById("reactieinput30").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam30;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie30;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput30").value = "";
    document.getElementById("reactieinput30").value = "";
  });
}
