...
document.addEventListener("DOMContentLoaded", async () => {
  const ervaringId26 = getQueryParam("id");
  ...
  if (!ervaringId26) {
    ...
    await laadEnToonErvaringEnReacties(ervaringId26);
    bindStapB(ervaringId26);
  }
});
...
function bindStapA() {
  ...
  plaatsenBtn.addEventListener("click", async (event) => {
    ...
    const emotie = document.getElementById("emotieinput26").value.trim();
    const ervaring = document.getElementById("ervaringinput26").value.trim();
    const htmlbestand = "stressmanblankkaal";
...
async function laadEnToonErvaringEnReacties(ervaringId26) {
  ...
  document.getElementById("toonEmotie26").textContent = ervaringRecord.emotie;
  document.getElementById("toonErvaring26").textContent = ervaringRecord.ervaring;

  const { data: reactiesData, error: errorReacties } = await supabase
    .from("reacties")
    .select("naam, reactie, created_at")
    .eq("ervaring_id", ervaringId26)
  ...
  const lijstContainer = document.getElementById("reactieLijst26");
  lijstContainer.innerHTML = "";
  if (reactiesData && reactiesData.length > 0) {
    reactiesData.forEach((item) => {
      const wrapper = document.createElement("div");
      wrapper.classList.add("reactietekst26");
...
function bindStapB(ervaringId26) {
  const placeBtntwee = document.getElementById("plaatsentweeButton");
  placeBtntwee.addEventListener("click", async (evt) => {
    evt.preventDefault();

    const naam26 = document.getElementById("naaminput26").value.trim();
    const reactie26 = document.getElementById("reactieinput26").value.trim();
    ...
    const naamEl = document.createElement("h3");
    naamEl.textContent = naam26;
    const tekstEl = document.createElement("p");
    tekstEl.textContent = reactie26;

    wrapper.appendChild(naamEl);
    wrapper.appendChild(tekstEl);
    lijstContainer.appendChild(wrapper);

    document.getElementById("naaminput26").value = "";
    document.getElementById("reactieinput26").value = "";
  });
}
